	...
	//  cout << "Creating new cluster with seed: " << pixel_hit->getPixel().getIndex().X()<< " " <<
	    ...
	    // Check if neighbor has been used or if it touches the current cluster:
            ...
            cluster.addPixelHit(neighbor);
                        ...
                        neighbor->getPixel().getIndex().Y() << " Max Pixel Charge: " << neighbor->getSignal()
                       << std::endl;
...
void PrintClusterMap(allpix::Cluster& cluster) {
  ...
  for(auto pixel : cluster.getPixelHits()) {
    auto pix_addr = pixel->getIndex();
    histo_iter++;
    if (histo_iter==1) cluster_1->SetBinContent(pix_addr.x(), pix_addr.y(), pixel->getSignal());
    else if (histo_iter==2) cluster_2->SetBinContent(pix_addr.x(), pix_addr.y(), pixel->getSignal());
    else if (histo_iter==3) cluster_3->SetBinContent(pix_addr.x(), pix_addr.y(), getSignal()); 
...
void AnalysisB10(string input_file_folder,
  ...
  // std::cout.rdbuf(out.rdbuf()); //redirect std::cout to out.txt!
    ...
    // TH1D* maxY_postion = new TH1D("maxY_position", "Max Charge Position in Y", 2500, 0, 14);
    
    
    TH2D* cluster_1 = new TH2D("cluster_1", "Cluster1 Pixels and Charge", 100, 0, 256, 100, 0, 256);
    TH2D* cluster_2 = new TH2D("cluster_2", "Cluster2 Pixels and Charge", 100, 0, 256, 100, 0, 256);
    TH2D* cluster_3 = new TH2D("cluster_3", "Cluster3 Pixels and Charge", 100, 0, 256, 100, 0, 256);
